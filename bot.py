...
def broadcast_message(message):
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, f"🔄 جاري إرسال الرسالة لـ {len(users)} مستخدم...")
        
        for user in users:
            try:
                bot.send_message(user[0], f"📢 إشعار من الإدارة:\n\n{broadcast_text}")
                sent_count += 1
                time.sleep(0.1)  # تجنب حظر التلغرام
            except:
                failed_count += 1
...
def incoming_message(message):
    try:
        ...
        if message.text:
            save_user_message(user.id, username, 'text', message.text)
        elif message.photo:
...
if __name__ == "__main__":
    try:
        ...
        bot.send_message(ADMIN_ID, "⏹️ تم إيقاف البوت")
    except Exception as e:
