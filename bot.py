...
def get_user_stats():
    try:
        ...
        cursor.execute('''
            SELECT user_id, username, MAX(created_at) 
            FROM user_messages 
            GROUP BY user_id 
        ...
        conn.close()
        return total_users, total_messages, recent_users
    except Exception as e:
...
def start(message):
    try:
        ...
        logger.info(f"✅ تم استقبال أمر start من المستخدم: {message.from_user.id}")
    except Exception as e:
...
def show_stats(message):
    ...
    try:
        total_users, total_messages, recent_users = get_user_stats()
...
"""
        for user in recent_users:
            stats_text += f"• ID: {user[0]} - @{user[1] or 'لا يوجد'}\n"
...
def broadcast_message(message):
    ...
    try:
        ...
        total_users, _, _ = get_user_stats()
        
        if total_users == 0:
            bot.send_message(ADMIN_ID, "❌ لا يوجد مستخدمين لإرسال الرسالة لهم")
            return
        
        bot.send_message(ADMIN_ID, f"🔄 جاري إرسال الرسالة لـ {total_users} مستخدم...")
        
        # في الإصدار الحقيقي، سنحتاج لجلب جميع المستخدمين من DB
        # لكن لتجنب التعقيد، سنستخدم طريقة مبسطة
        sent_count = 0
        failed_count = 0
        
        # ملاحظة: في التطبيق الحقيقي، تحتاج لجلب جميع user_ids من DB
        # conn = sqlite3.connect('bot_data.db', check_same_thread=False)
        # cursor = conn.cursor()
        # cursor.execute('SELECT DISTINCT user_id FROM user_messages')
...
✅ تم الانتهاء من البث (وهمي - يحتاج تطوير):
✔ تم الإرسال: {sent_count}
❌ فشل الإرسال: {failed_count}
        """)
        
        logger.info(f"📢 تم بث رسالة لـ {sent_count} مستخدم")
...
def incoming_message(message):
    try:
        ...
        if message.text:
            save_user_message(user.id, username, 'text', message.text[:500])  # تقليل الطول
        elif message.photo:
